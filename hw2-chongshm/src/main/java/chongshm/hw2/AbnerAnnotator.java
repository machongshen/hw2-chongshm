...
/**
 * Description: By using the Abner API as NER to generate gene tag. In the
 * meantime, we also compare the output of Abner, and use the regular expression
 * to exclude the inconceivable data. Then save the data to the CAS.
 * 
...
public class AbnerAnnotator extends JCasAnnotator_ImplBase {
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		for (int i = 0; i < m[0].length; i++) {
			...
			double sb = 0.61d;
			/**
			 * Description: use the regular expression and irrelative
			 * information to exclude the inconceivable data. This method helps
			 * me improve the precision of the extracting gene tag.
			 * 
			 */
			if (k[1].indexOf(extract) != -1 && extract2 != "CELL_TYPE"
					&& extract2 != "RNA" && extract.length() > 1) {
